...
- Notification access: reads active Android-standard message and email notifications in memory so they can be grouped and replied to through the originating app when it supplies a compatible reply action. Other notification categories are ignored, and conversation history is not stored by MinkLauncher Open.
- Usage access: reads foreground events for the social apps the user chooses to track while MinkLauncher Open is visible. Other apps are excluded from the trail and totals. The app stores the user’s goal and app choices, but does not create a separate usage-history database.
